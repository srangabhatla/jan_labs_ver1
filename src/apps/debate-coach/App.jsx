...
function DebateCoachApp() {
  ...
  async function generateSides() {
    ...
    try {
      ...
      const raw  = await callClaude(system, user, 900, "debate-coach");
  ...
  async function fetchFirstChallenge(t, uPos, oPos, roundNum) {
    ...
    try {
      ...
      const raw  = await callClaude(system, user, 400, "debate-coach");
      const data = parseJSON(raw);
  ...
  async function submitRebuttal() {
    ...
    try {
      ...
      if (isLastRound) {
        ...
        const raw  = await callClaude(system, user, 600, "debate-coach");
        const data = parseJSON(raw);
...
export default function DebateCoach() {
  return (
    <AuthWrapper appId="debate-coach">
      <DebateCoachApp />
    </AuthWrapper>
  );
}
